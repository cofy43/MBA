...
to central
  ; Encender solo el pathc central
  ask patch 0 max-pycor [set pcolor orange]
end

to aleatorio
  ; Encender aleatoriamente los patch iniciales
  let max-x-patches max-pxcor * 2
  while [max-x-patches >= 0] [
    let proba (random-float 1)
    show proba
    show max-x-patches
    if proba > 0.6 [
      ask patch max-x-patches max-pycor [set pcolor orange]
    ]
    set max-x-patches (max-x-patches - 1)
  ]
end
...
to decimal-2-binary
  let temp-decimal decimal
  set ooo (temp-decimal mod 2)
  set temp-decimal floor(temp-decimal / 2)
  set ooi (temp-decimal mod 2)
  set temp-decimal floor(temp-decimal / 2)
  set oio (temp-decimal mod 2)
  set temp-decimal floor(temp-decimal / 2)
  set oii (temp-decimal mod 2)
  set temp-decimal floor(temp-decimal / 2)
  set ioo (temp-decimal mod 2)
  set temp-decimal floor(temp-decimal / 2)
  set ioi (temp-decimal mod 2)
  set temp-decimal floor(temp-decimal / 2)
  set iio (temp-decimal mod 2)
  set temp-decimal floor(temp-decimal / 2)
  set iii (temp-decimal mod 2)
end
...
164
46
Número de la regla (Entre 0-255):
11
0.0
1
...
715
52
Representación de la Regla seleccionada por el usuario en binario:
11
0.0
1
...
183
517
Instrucciones:\n * Primero seleccionar el número de la regla con el slider\n * Después seleccionar el botón setup\n * Seleccionar el tipo de inicialización\n * Finalmente precionar el botón go
11
0.0
1
