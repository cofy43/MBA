to setup
  ...
  ask patches [
    if random-float 100 < densidad_astillas_amarillas [
      set pcolor yellow
    ]
   if random-float 100 < densidad_cafes [
      set pcolor brown
    ]
  ]
  ;; randomly distribute termites
  create-turtles num_termitas [
    set color white
...
to draw-plot
  set-current-plot "couster tamanio 4"
  plot count patches with
  [ count neighbors4 with [ pcolor = yellow] = 4 ]
  ;plot count patches with
  ;[ count neighbors4 with [ pcolor = brown] = 4 ]
end
@#$#@#$#@
GRAPHICS-WINDOW
326
