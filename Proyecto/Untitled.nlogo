...
to go
  ...
  ask turtles with [color = red] [
    ...
    ; Verificamos que no entremos al vagon por alguna separacion
    if pcolor = white [
      rt random 180
      fd 1
    ]
    ; Verificamos si estamos parados en una zona de espera
    if pcolor = yellow [
      set fila true
    ]
    ; Verificamos si estamos en alguna puerta
    if pcolor = green [
      set meta true
    ]
    ; Si ya cruzamos la puerta solo avanzamos en linea recta hacia el norte
    if meta [
      set heading 0
      if ycor < 17 [ fd 1 ]
    ]
...
to dibuja-vagon
  ; Contador del número de patches pintados
  let tempCount 0
  ; Contador del número de salidas dibujadas
  let tempCountExit 0
  ; Contadores de las coordenadas donde estamos parados
  ; en la iteracion
  let x 0
  let y 0
  ; Bandera para saber si cambiamos de color
  let printMargin true

  ; Tamaño total de patches para dibujar las salidas
...
"" ""
PENS
"default" 1.0 0 -13791810 true "" "plot count turtles with [color = blue and ycor < 8]"
"pen-1" 1.0 0 -8053223 true "" "plot count turtles with [color = red and ycor < 8]"
